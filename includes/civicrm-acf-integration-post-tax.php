...
/**
 * CiviCRM ACF Integration Post Taxonomy Class.
 *
 * A class that encapsulates Post Taxonomy functionality.
 *
 * @since 0.6.4
 */
class CiviCRM_ACF_Integration_Post_Tax {
	...
	public function __construct( $parent ) {
		...
		add_action( 'civicrm_acf_integration_post_loaded', [ $this, 'register_hooks' ] );
	...
	public function form_element_add_term_add( $taxonomy ) {
		...
		include CIVICRM_ACF_INTEGRATION_PATH . 'assets/templates/wordpress/term-add.php';
	...
	public function form_element_edit_term_add( $tag, $taxonomy ) {
		...
		// Include template file.
		include CIVICRM_ACF_INTEGRATION_PATH . 'assets/templates/wordpress/term-edit.php';
	...
	public function form_element_get_groups( $taxonomy, $group_id = null ) {
		...
		$groups_all = $this->plugin->civicrm->group->groups_get_all();
	...
	public function synced_terms_get_for_post_and_group( $post_id, $group_id ) {
		...
		*/
		...
		if ( ! $this->post->is_mapped( $post_id ) ) {
	...
	public function post_saved( $args ) {
		...
		if ( ! empty( $group_ids_added ) ) {
			foreach( $group_ids_added AS $term_id => $group_id ) {
				...
				$is_member = $this->plugin->civicrm->group->group_contact_exists( $group_id, $args['contact_id'] );
				if ( $is_member === false ) {

					// Add to the Group.
					$this->plugin->civicrm->group->group_contact_create( $group_id, $args['contact_id'] );
					 ...
					 */
					do_action( 'civicrm_acf_integration_post_tax_group_contact_created', $args, $group_id, $term_id );
		...
		if ( ! empty( $group_ids_removed ) ) {
			foreach( $group_ids_removed AS $term_id => $group_id ) {
				...
				if ( $is_member === true ) {
					...
					$this->plugin->civicrm->group->group_contact_delete( $group_id, $args['contact_id'] );
					 ...
					 * @param integer $term_id The ID of the term.
					 */
					do_action( 'civicrm_acf_integration_post_tax_group_contact_deleted', $args, $group_id, $term_id );
		 ...
		 * @param array $args The array of data.
		 */
		do_action( 'civicrm_acf_integration_post_tax_post_saved', $args );
