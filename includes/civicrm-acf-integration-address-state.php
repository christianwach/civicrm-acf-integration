...
/**
 * CiviCRM ACF Integration CiviCRM State Class.
 *
 * A class that encapsulates CiviCRM State functionality.
 *
 * @since 0.8.3
 */
class CiviCRM_ACF_Integration_CiviCRM_Address_State extends CiviCRM_ACF_Integration_CiviCRM_Base {
	...
	public function address_process( $address, $args ) {
		 ...
		 * @param array $args The array of CiviCRM params.
		 */
		do_action( 'civicrm_acf_integration_address_state_update', $contact, $address, $args );
	...
	public function fields_update( $post_id, $address, $args ) {
		...
		foreach( $acf_fields['state'] AS $selector => $address_field ) {
			...
			// Now update Field.
			$this->plugin->acf->field->value_update( $selector, $value, $post_id );
