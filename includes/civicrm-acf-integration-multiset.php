...
/**
 * CiviCRM ACF Integration CiviCRM Multiple Record Set Class.
 *
 * A class that encapsulates CiviCRM Multiple Record Set functionality.
 *
 * @since 0.8
 */
class CiviCRM_ACF_Integration_CiviCRM_Multiple_Record_Set extends CiviCRM_ACF_Integration_CiviCRM_Base {
	...
	public function register_hooks() {
		...
		add_action( 'civicrm_acf_integration_mapper_multiset_created', [ $this, 'multiset_edited' ], 10, 1 );
		add_action( 'civicrm_acf_integration_mapper_multiset_edited', [ $this, 'multiset_edited' ], 10, 1 );

		// Intercept when a CiviCRM Multiple Record Set is being deleted.
		add_action( 'civicrm_acf_integration_mapper_multiset_pre_delete', [ $this, 'multiset_pre_delete' ], 10, 1 );
		add_action( 'civicrm_acf_integration_mapper_multiset_deleted', [ $this, 'multiset_deleted' ], 10, 1 );
		...
		add_action( 'civicrm_acf_integration_multiset_created', [ $this, 'maybe_sync_multiset_id' ], 10, 2 );
		*/
	 ...
	 * @since 0.8
	 *
	 * @param int $multiset_id The numeric ID of the Multiple Record Set.
	...
	public function multisets_update( $values, $contact_id, $selector, $args = [] ) {
		...
		if ( empty( $current ) ) {
			...
			foreach( $values AS $key => $value ) {
				 ...
				 * @param array $args The array of WordPress params.
				 */
				do_action( 'civicrm_acf_integration_multiset_created', $params, $args );
		...
		foreach( $actions['update'] AS $key => $value ) {
			 ...
			 * @param array $args The array of WordPress params.
			 */
			do_action( 'civicrm_acf_integration_multiset_updated', $params, $args );
		...
		foreach( $actions['delete'] AS $multiset_id ) {
			 ...
			 * @param array $args The array of WordPress params.
			 */
			do_action( 'civicrm_acf_integration_multiset_deleted', $params, $args );
	 ...
	 * @param int $multiset_id The numeric ID of the Multiple Record Set (or null if new).
	 ...
	 *
	 * @param int $contact_id The numeric ID of the Contact.
	 * @param str $data The Multiple Record Set data to update the Contact with.
			...
			foreach( $acf_fields['multiset'] AS $selector => $multiset_field ) {
				...
				switch( $args['op'] ) {
					...
					case 'create' :
						...
						if ( $acf_multiset['field_multiset_primary'] == '1' AND ! empty( $existing ) ) {
	...
	public function get_for_acf_field( $field ) {
		 ...
		 * @param bool|array $mapped An array of Post Types if the Field Group is mapped, or false if not mapped.
		 */
	...
	public function acf_field_get( $custom_groups = [] ) {
		...
		*/
		 ...
		 */
		$choices = apply_filters( 'civicrm_acf_integration_civicrm_multiset_choices', $choices );
