...
/**
 * CiviCRM ACF Integration Custom ACF Field Type - CiviCRM Phone Field.
 *
 * A class that encapsulates a "CiviCRM Phone Field" Custom ACF Field in ACF 5+.
 *
 * @since 0.7.3
 */
class CiviCRM_ACF_Integration_Custom_CiviCRM_Phone_Field extends acf_field {
	...
	public function __construct( $parent ) {
		...
		$this->label = __( 'CiviCRM Phone', 'civicrm-acf-integration' );
	...
	public function render_field_settings( $field ) {
		...
		$setting = [
			'label' => __( 'CiviCRM Phone ID', 'civicrm-acf-integration' ),
			'name' => 'show_phone_id',
			'type' => 'true_false',
			'ui' => 1,
	...
	public function validate_value( $valid, $value, $field, $input ) {
		...
		if ( ! in_array( 1, $primary_values ) ) {
			$valid = __( 'Please select a Primary Number', 'civicrm-acf-integration' );
			return $valid;
		}
		...
		if ( in_array( '', $phones ) ) {
			$valid = __( 'Please enter a Phone Number', 'civicrm-acf-integration' );
			return $valid;
		}
	...
	public function modify_field( $field ) {
		...
		$field['layout'] = 'table';
		$field['button_label'] = __( 'Add Phone Number', 'civicrm-acf-integration' );
		$field['collapsed'] = '';
		$field['prefix'] = '';
		...
		$number = [
			...
			'label' => __( 'Phone Number', 'civicrm-acf-integration' ),
			'name' => 'phone_number',
			'type' => 'text',
			'parent' => $field['key'],
			'instructions' => '',
		...
		$extension = [
			...
			'label' => __( 'Extension', 'civicrm-acf-integration' ),
			'name' => 'extension',
			'type' => 'text',
			'parent' => $field['key'],
			'instructions' => '',
		...
		$type = [
			...
			'label' => __( 'Type', 'civicrm-acf-integration' ),
			'name' => 'type',
			'type' => 'select',
			'parent' => $field['key'],
			'instructions' => '',
